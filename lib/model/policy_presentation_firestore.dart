...
/* 
 * PolicyPresentationFirestore is the firestore implementation of PolicyPresentationRepository
 */
class PolicyPresentationFirestore implements PolicyPresentationRepository {
  ...
  PolicyPresentationEntity? fromMap(Object? o,
    ...
    return PolicyPresentationEntity.fromMap(o, newDocumentIds: newDocumentIds);
  }
  ...
  Future<PolicyPresentationModel?> _populateDoc(DocumentSnapshot value) async {
    return PolicyPresentationModel.fromEntity(
        value.id, PolicyPresentationEntity.fromMap(value.data()));
  }

  Future<PolicyPresentationModel?> _populateDocPlus(
      DocumentSnapshot value) async {
    return PolicyPresentationModel.fromEntityPlus(
        value.id, PolicyPresentationEntity.fromMap(value.data()),
        appId: appId);
  }
...
//  stream = getQuery(policyPresentationCollection, orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots()
  ...
  StreamSubscription<PolicyPresentationModel?> listenTo(
    ...
    var stream = policyPresentationCollection
        .doc(documentId)
        .snapshots()
        .asyncMap((data) {
  ...
  Stream<List<PolicyPresentationModel?>> values(
    ...
    Stream<List<PolicyPresentationModel?>> values = getQuery(
            policyPresentationCollection,
            orderBy: orderBy,
            descending: descending,
            startAfter: startAfter as DocumentSnapshot?,
            limit: limit,
  ...
  Future<List<PolicyPresentationModel?>> valuesList(
    ...
    DocumentSnapshot? lastDoc;
    List<PolicyPresentationModel?> values = await getQuery(
            policyPresentationCollection,
            orderBy: orderBy,
            descending: descending,
            startAfter: startAfter as DocumentSnapshot?,
  ...
  final String appId;
  PolicyPresentationFirestore(this.getCollection, this.appId)
      : policyPresentationCollection = getCollection();
