...
import 'member_action_form.dart';


typedef MemberActionWidgetProvider(MemberActionModel? value);

class MemberActionListWidget extends StatefulWidget with HasFab {
  AppModel app;
  BackgroundModel? listBackground;
  MemberActionWidgetProvider? widgetProvider;
  bool? readOnly;
  String? form;
  MemberActionListWidgetState? state;
  bool? isEmbedded;

  MemberActionListWidget({ Key? key, required this.app, this.readOnly, this.form, this.widgetProvider, this.isEmbedded, this.listBackground }): super(key: key);

  @override
  MemberActionListWidgetState createState() {
    state ??= MemberActionListWidgetState();
    return state!;
  }
...
class MemberActionListWidgetState extends State<MemberActionListWidget> {
  @override
      ...
      onPressed: () {
        Navigator.of(context).push(
          pageRouteBuilder(widget.app, page: BlocProvider.value(
              value: BlocProvider.of<MemberActionListBloc>(context),
              child: MemberActionForm(app:widget.app,
                  value: null,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AccessBloc, AccessState>(
      ...
      if (accessState is AccessDetermined) {
        return BlocBuilder<MemberActionListBloc, MemberActionListState>(builder: (context, state) {
          if (state is MemberActionListLoading) {
            return StyleRegistry.registry().styleWithApp(widget.app).adminListStyle().progressIndicator(widget.app, context);
          } else if (state is MemberActionListLoaded) {
    ...
    return Container(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return MemberActionListItem(app: widget.app,
            value: value,
//            app: accessState.app,
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return MemberActionForm(app:widget.app, value: value, formAction: action);
    } else {
      return null;
    }
  }
...
class MemberActionListItem extends StatelessWidget {
  final AppModel app;
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      key: Key('__MemberAction_item_${value.documentID}'),
